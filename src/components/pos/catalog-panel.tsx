...
export function CatalogPanel({ onSelect, collapsed, onToggleCollapsed }: CatalogPanelProps) {
  ...
  useEffect(() => {
    const q = search.trim();
    if (!q) return;
    const match = products.find((p) => (p.sku && p.sku === q) || (p.barcode && p.barcode === q));
    if (match) {
      setSearch("");
      onSelect(match);
    }
  ...
  if (collapsed) {
    return (
      <div className="flex h-full flex-col gap-2 p-3">
        <Button variant="outline" size="sm" onClick={onToggleCollapsed} className="justify-start">
  ...
  const withCount = [{ id: "", name: "Todos", imageUrl: null, productCount: products.length }, ...categories];
  ...
  return (
    <div className="flex h-full flex-col gap-3 p-3">
      <div className="flex items-center gap-2">
        ...
        <Button variant="ghost" size="sm" onClick={onToggleCollapsed} aria-label="Contraer productos">
      ...
      <div className="scrollbar-none flex shrink-0 items-center gap-1.5 overflow-x-auto pb-1">
        {withCount.map((c) => (
          ...
          >
            {c.name}
            <span className={cn("ml-1 opacity-60", activeCategory === (c.id || null) && "text-primary-foreground")}>
              {c.productCount}
            </span>
          </button>
        ))}
