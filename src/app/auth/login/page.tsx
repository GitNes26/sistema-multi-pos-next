...
export default async function PosLoginPage({
  ...
  if (session?.user) {
    redirect(
      session.user.scope === "portal" ? "/portal" : session.user.scope === "superadmin" || session.user.role === "manager" || session.user.role === "owner" ? "/admin" : "/pos"
    );
  }
    ...
    <AuthShell
      ...
      footerLinks={
        <>
          ...
          <Link href="/auth/forgot" className="hover:text-foreground hover:underline">
