  ...
  try {
    ...
    if (allOrgs) {
      ...
      const credits = rows.map((r) => ({
        ...
        customCreditLimit: r.creditLimit != null ? Number(r.creditLimit) : null,
        useDefaultLimit: r.useDefaultLimit,
...
// PATCH /api/customer-credit → límite heredado/personalizado y bloqueo operativo
export async function PATCH(req: Request) {
  ...
  try {
    const body = await req.json()
    const { customerId, creditLimit, useDefaultLimit, status } = body
    ...
    if (status !== undefined && status !== "active" && status !== "suspended")
      return NextResponse.json(
        { ok: false, error: "Estado de crédito inválido" },
        { status: 400 }
      )
    if (useDefaultLimit === undefined && status === undefined)
      ...
      await setCustomerCreditSettings(guard.organizationId, customerId, {
        useDefaultLimit:
          useDefaultLimit === undefined ? undefined : Boolean(useDefaultLimit),
        creditLimit:
