...
const NON_ASSIGNABLE_SHARED_IDS = new Set([
  "system-superadmin",
  "system-customer",
])
...
export async function createOrganization(
  ...
  const org = await prisma.$transaction(async (tx) => {
    ...
    await tx.membership.upsert({
      where: {
        userId_organizationId: { userId: owner.id, organizationId: created.id },
      },
      update: { role: "owner" },
...
export async function deleteOrganization(
  ...
  await prisma.$transaction(async (tx) => {
    ...
    const prepIds = (
      await tx.orderPreparation.findMany({
        where: { orderId: { in: orderIds } },
        select: { id: true },
      })
    ).map((o) => o.id)
    await tx.orderPreparationItem.deleteMany({
      where: { preparationId: { in: prepIds } },
    })
    await tx.orderPreparation.deleteMany({
      where: { orderId: { in: orderIds } },
    })
    ...
    const revisionIds = (
      await tx.inventoryRevision.findMany({
        where: { organizationId },
        select: { id: true },
      })
    ).map((r) => r.id)
    await tx.inventoryRevisionItem.deleteMany({
