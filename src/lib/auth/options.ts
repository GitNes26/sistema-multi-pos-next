...
export type BusinessMode =
  "retail" | "food_service" | "services" | "rental" | "hybrid"
...
function inferKindFromUser(
  user: NonNullable<Awaited<ReturnType<typeof resolveLoginUser>>>
) {
  // SuperAdmin por rol (flag), no por ausencia de empleado/cliente.
  if (user.isSuperadmin) {
    return {
      scope: "superadmin" as AuthScope,
  ...
  const org =
    ...
    user.customers[0]?.organizationId ??
    null
      ...
      async authorize(credentials) {
        ...
        if (
          ...
          try {
            ...
            console.error(
              "[auth] no se pudo recordar la organización del login",
              err
            )
          }
        }
  ...
  callbacks: {
    async jwt({ token, user, trigger, session: updatePayload }) {
      ...
      if (
        trigger === "update" &&
        updatePayload &&
        "activeOrganizationId" in updatePayload
      ) {
        const next = (updatePayload as { activeOrganizationId?: string | null })
          .activeOrganizationId
        if (token.scope === "portal" && next) {
          const customerAccess = await prisma.customer.findFirst({
            where: { userId: token.id, organizationId: next, isActive: true },
            select: { id: true },
          })
        ...
        if (next) {
          ...
          if (membership) {
            ...
            token.permissions = await permissionsForRole(
              role,
              membership.roleId,
              next
            )
        ...
        try {
          ...
          console.error(
            "[auth] no se pudo recordar la organización activa",
            err
          )
        }
      }
