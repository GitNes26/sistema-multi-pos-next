...
if (!["demo", "minimal"].includes(profile)) throw new Error("Profile must be demo or minimal.");
...
if (mode === "prepare" && !fs.existsSync(envFile)) {
  ...
  if (!["localhost", "127.0.0.1"].includes(original.hostname)) throw new Error("Automatic setup only supports local MySQL.");
  const databaseName = `multi_pos_test_${profile}_${Date.now()}`;
  const { PrismaClient } = require("@prisma/client");
  const admin = new PrismaClient({ datasourceUrl: original.href });
  try { await admin.$executeRawUnsafe(`CREATE DATABASE \`${databaseName}\` CHARACTER SET utf8mb4 COLLATE utf8mb4_unicode_ci`); }
  ...
  fs.writeFileSync(envFile, JSON.stringify({
    ...
    TWILIO_ACCOUNT_SID: "", TWILIO_AUTH_TOKEN: "", VAPID_PRIVATE_KEY: "",
  }, null, 2));
}
...
function run(bin, args, extra = {}) {
  const r = spawnSync(process.execPath, [require.resolve(bin), ...args], { cwd: root, env: { ...env, ...extra }, stdio: "inherit" });
  if (r.error) throw r.error;
  if (r.status !== 0) process.exit(r.status ?? 1);
}
switch (mode) {
  case "schema-diff":
    run("prisma/build/index.js", ["migrate", "diff", "--from-schema-datasource", "prisma/schema.prisma", "--to-schema-datamodel", "prisma/schema.prisma", "--script"]);
    break;
  case "prepare":
    ...
    break;
  case "serve":
    run("next/dist/bin/next", ["dev", "--port", new URL(env.TEST_BASE_URL).port], { NODE_ENV: "development" });
    break;
  case "integration":
    { const r = spawnSync(process.execPath, ["--test", "tests/permissions.test.mjs"], { cwd: root, env, stdio: "inherit" }); process.exit(r.status ?? 1); }
    break;
  case "e2e":
    run("@playwright/test/cli", ["test", ...process.argv.slice(4)]);
    break;
  default: throw new Error("Use prepare, serve, integration or e2e.");
}
